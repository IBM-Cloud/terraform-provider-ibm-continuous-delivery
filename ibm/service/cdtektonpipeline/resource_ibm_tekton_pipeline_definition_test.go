		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMTektonPipelineDefinitionExists("ibmcd_tekton_pipeline_definition.tekton_pipeline_definition", conf),
					resource.TestCheckResourceAttr("ibmcd_tekton_pipeline_definition.tekton_pipeline_definition", "pipeline_id", pipelineID),
				),
			},
			resource.TestStep{
				ResourceName:      "ibmcd_tekton_pipeline_definition.tekton_pipeline_definition",
				ImportState:       true,
...
func testAccCheckIBMTektonPipelineDefinitionDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "ibmcd_tekton_pipeline_definition" {
			continue
		}
