		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckIBMTektonPipelineTriggerExists("ibmcd_tekton_pipeline_trigger.tekton_pipeline_trigger", conf),
					resource.TestCheckResourceAttr("ibmcd_tekton_pipeline_trigger.tekton_pipeline_trigger", "pipeline_id", pipelineID),
				),
			},
			resource.TestStep{
				ResourceName:      "ibmcd_tekton_pipeline_trigger.tekton_pipeline_trigger",
				ImportState:       true,
...
func testAccCheckIBMTektonPipelineTriggerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "ibmcd_tekton_pipeline_trigger" {
			continue
		}
